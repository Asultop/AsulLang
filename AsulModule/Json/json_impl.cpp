// 这个文件包含json package的实现，会被ALangEngine.cpp直接包含
// 这样就能访问ALangEngine.cpp中匿名命名空间的类型

// 直接实现initialize函数，不需要命名空间
void initializeJsonPackage(std::shared_ptr<Object> jsonPkg) {
