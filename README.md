...
ALang 正在持续发展，计划实现以下高级语言特性：

### 即将推出 (Upcoming Features)
- 🔧 **FFI 增强**: libffi 集成，支持复杂类型传递（数组、对象、结构体）
- 📦 **解构赋值**: `let [a, b] = array` 和 `let {x, y} = object`
- ⛓️ **可选链**: `obj?.prop?.method?.()`，安全访问可能为空的属性
- 🔄 **生成器/yield**: 支持惰性求值和无限序列
- 🎨 **装饰器**: `@decorator` 语法，元编程支持
- 🔀 **模式匹配**: `match/case` 结构化匹配，比 switch 更强大

### 详细规划
完整的特性设计、示例代码和实现计划请参阅：
- 📖 [特性路线图文档](FEATURE_ROADMAP.md)
