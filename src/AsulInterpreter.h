		...
		if (auto parr = std::get_if<std::shared_ptr<Array>>(&obj)) {
			...
			if (name == "flat") {
				...
				fn->builtin = [this,a](const std::vector<Value>& args, std::shared_ptr<Environment>)->Value {
					if (!a) return Value{std::monostate{}};
					int depth = 1;
					if (!args.empty()) depth = static_cast<int>(getNumber(args[0], "flat depth"));
			...
			if (name == "flatMap") {
				...
				fn->builtin = [this,a](const std::vector<Value>& args, std::shared_ptr<Environment>)->Value {
					if (!a) return Value{std::monostate{}};
					if (args.size() != 1 || !std::holds_alternative<std::shared_ptr<Function>>(args[0])) throw std::runtime_error("flatMap expects a single function argument");
					auto cb = std::get<std::shared_ptr<Function>>(args[0]);
					auto out = std::make_shared<Array>();
					for (size_t i = 0; i < a->size(); ++i) {
						Value elem = (*a)[i];
			...
			if (name == "groupBy") {
				...
				fn->builtin = [this,a](const std::vector<Value>& args, std::shared_ptr<Environment>)->Value {
					if (!a) return Value{std::monostate{}};
					if (args.size() != 1 || !std::holds_alternative<std::shared_ptr<Function>>(args[0])) throw std::runtime_error("groupBy expects a single function argument");
					auto cb = std::get<std::shared_ptr<Function>>(args[0]);
					...
					for (size_t i = 0; i < a->size(); ++i) {
						...
						if (cb->isBuiltin) {
							...
							if (cb->params.size() > 1) local->define(cb->params[1], Value{ static_cast<double>(static_cast<int>(i)) });
							if (cb->params.size() > 2) local->define(cb->params[2], Value{a});
							try { executeBlock(cb->body, local); } catch (const ReturnSignal& rs) { key = rs.value; }
						}
			...
			if (name == "zip") {
				...
				fn->builtin = [this,a](const std::vector<Value>& args, std::shared_ptr<Environment>)->Value {
					...
					for (const auto& arg : args) {
						if (!std::holds_alternative<std::shared_ptr<Array>>(arg)) throw std::runtime_error("zip expects array arguments");
						arrays.push_back(std::get<std::shared_ptr<Array>>(arg));
					}
					size_t minLen = a->size();
			...
			if (name == "diff") {
				...
				fn->builtin = [this,a](const std::vector<Value>& args, std::shared_ptr<Environment>)->Value {
					if (!a) return Value{std::monostate{}};
					if (args.empty()) return Value{a};
					if (!std::holds_alternative<std::shared_ptr<Array>>(args[0])) throw std::runtime_error("diff expects array argument");
					auto other = std::get<std::shared_ptr<Array>>(args[0]);
					auto out = std::make_shared<Array>();
					for (const auto& elem : *a) {
						bool found = false;
						for (const auto& otherElem : *other) {
