...
public:
	...
	Value getProperty(const Value& obj, const std::string& name) {
		...
		if (auto parr = std::get_if<std::shared_ptr<Array>>(&obj)) {
			...
			if (name == "slice") { auto fn = std::make_shared<Function>(); fn->isBuiltin = true; auto a=*parr; fn->builtin=[a](const std::vector<Value>& args, std::shared_ptr<Environment>)->Value { if (!a) return Value{std::monostate{}}; size_t n=a->size(); double s=0; double e=n; if (!args.empty()) s=getNumber(args[0], "slice start"); if (args.size()>=2) e=getNumber(args[1], "slice end"); if (s<0) s=0; if (e<0) e=0; size_t si=static_cast<size_t>(std::max(0.0,s)); size_t ei=static_cast<size_t>(std::max(0.0,e)); if (si>n) si=n; if (ei>n) ei=n; if (ei<si) ei=si; auto out=std::make_shared<Array>(); for (size_t i=si;i<ei;++i) out->push_back((*a)[i]); return Value{out}; }; return fn; }
			if (name == "indexOf") { auto fn=std::make_shared<Function>(); fn->isBuiltin=true; auto a=*parr; fn->builtin=[a](const std::vector<Value>& args, std::shared_ptr<Environment>)->Value { if (!a) return Value{-1.0}; if (args.empty()) throw std::runtime_error("indexOf expects value argument"); size_t start=0; if (args.size()>=2) { double sd=getNumber(args[1], "indexOf start"); if (sd<0) sd=0; start=static_cast<size_t>(sd); if (start>a->size()) return Value{-1.0}; } for (size_t i=start;i<a->size();++i) { if (valueEqual((*a)[i], args[0])) return Value{ static_cast<double>(i) }; } return Value{-1.0}; }; return fn; }
			...
			if (name == "splice") { auto fn=std::make_shared<Function>(); fn->isBuiltin=true; auto a=*parr; fn->builtin=[a](const std::vector<Value>& args, std::shared_ptr<Environment>)->Value { if (!a) return Value{std::monostate{}}; if (args.empty()) throw std::runtime_error("splice expects start index"); double startD=getNumber(args[0], "splice start"); size_t start = static_cast<size_t>(std::max(0.0,startD)); if (start>a->size()) start=a->size(); size_t deleteCount=0; size_t insertFrom=1; if (args.size()>=2) { double delD=getNumber(args[1], "splice deleteCount"); if (delD<0) delD=0; deleteCount=static_cast<size_t>(delD); insertFrom=2; } if (start+deleteCount>a->size()) deleteCount = a->size()-start; auto removed=std::make_shared<Array>(); for (size_t i=0;i<deleteCount;++i) removed->push_back((*a)[start+i]); a->erase(a->begin()+start, a->begin()+start+deleteCount); // insert new items
