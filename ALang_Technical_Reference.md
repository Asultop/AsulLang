...
| **比较** | `==`, `!=`, `===`, `!==`, `<`, `>`, `<=`, `>=` | 相等性与关系比较 |
| **逻辑** | `&&`, `||`, `!` | 逻辑与、或、非 |
| **位运算** | `~`, `&`, `|`, `^`, `<<`, `>>` | 按位非、与、或、异或、左移、右移（均对 number 转 64 位整数后操作） |
...
注意：由于目前实现顺序，位运算低于比较和相等；编写如 `a | b == 15` 时请使用括号 `(a | b) == 15` 以确保意图明确。后续可根据需要调整为更常见的 JS/C 风格优先级。
